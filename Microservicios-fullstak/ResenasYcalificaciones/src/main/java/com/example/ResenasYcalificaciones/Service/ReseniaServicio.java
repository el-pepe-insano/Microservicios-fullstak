...
public class ReseniaServicio {
    @Autowired private ReseniaRepository repositorio;
    ...
    public Resenia guardar(Resenia resenia) {
        ...
        Usuario usuario = usuarioClienteService.obtenerUsuarioPorId(resenia.getIdCliente()).block();
        if (usuario == null)
            throw new RuntimeException("El usuario con ID " + resenia.getIdCliente() + " no existe.");
        return repositorio.save(resenia);
    }

    public List<Resenia> buscarTodas() { return repositorio.findAll(); }
    ...
    public List<Resenia> buscarPorCliente(Long idCliente) {
        return repositorio.findByIdCliente(idCliente);
    }

    public double calcularPromedio(Long idProducto) {
        List<Resenia> resenias = repositorio.findByIdProducto(idProducto);
        if (resenias.isEmpty()) throw new RuntimeException("No hay reseñas para el producto " + idProducto);
        return resenias.stream().mapToInt(Resenia::getCalificacion).average().orElse(0.0);
    }

    public boolean verificarUsuario(Long idUsuario) {
        Usuario usuario = usuarioClienteService.obtenerUsuarioPorId(idUsuario).block();
        return usuario != null;
    }

    public void eliminar(Long id) {
        if (id == null || id <= 0) throw new RuntimeException("El ID de la reseña debe ser válido.");
        if (!repositorio.existsById(id)) throw new RuntimeException("No se encontró una reseña con el ID especificado.");
